...
# tabClassification displays a box that you can write your text to cllasify.
# tabMain displays the main app.

MainTab, TextClassificationTab, MajorUptade, BugFix = st.tabs(["Main", "Text Classification", "Major Uptade", 'Bug Fix'])

with TextClassificationTab:
...
with MainTab:
    ...
    def main():
        """Reads data, creates a customizable horizontal bar chart, and displays it."""
        ...
        try:
            ...
            st.error(
                " 'etiketli_veri.csv' file not found. Please ensure it exists in the same directory as your script.")
            exit(1)

        # Check for columns
        if "zero_shot_label" not in df.columns or "zero_shot_score" not in df.columns:
            st.error(
                "Columns 'zero_shot_label' and 'zero_shot_score' not found. Please ensure they exist in your data.")
            exit(1)
